...
function ChildSnapshotInner() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error.message || t('Failed to load child information.'));
      setSnapshot(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDownload = async (report: ChildReportCard) => {
    ...
    try {
      ...
      if (result.kind === 'downloaded') {
        toast.success(t('Report card downloaded.'));
      } else if (result.kind === 'processing') {
        toast(result.message || t('The report card is still being generated — try again in a moment.'), { icon: '⏳' });
      } else if (result.kind === 'fee-blocked') {
        toast.error(
          result.shortfall
            ? `${t('Report card locked: outstanding fees of')} ${Number(result.shortfall).toLocaleString()} XAF. ${t('Please settle with the Bursar.')}`
            : result.message || t('Report card locked until school fees are settled.'),
          { duration: 8000 }
        );
      } else {
  ...
  if (!matricule) {
    return (
      <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{t('Results & Report Cards')}</h1>
          <p className="text-sm text-gray-500 mt-1">{t('Choose a child to see their full academic snapshot.')}</p>
        </div>
        <div className="bg-white rounded-lg shadow divide-y divide-gray-100">
          {children.length === 0 ? (
            <p className="p-6 text-sm text-gray-400 text-center">{t('No children linked to your account yet.')}</p>
          ) : (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {child.matricule || t('No matricule on file')}
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-4">
      <div className="flex items-center justify-between gap-2">
        ...
        >
          <ArrowLeftIcon className="w-4 h-4" /> {t('All children')}
        </button>
        <button onClick={load} className="p-2 text-gray-500 hover:bg-gray-100 rounded-md" title={t('Refresh')}>
          <ArrowPathIcon className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`} />
        </button>
      </div>

      {isLoading && !snapshot ? (
        <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">{t('Loading snapshot…')}</div>
      ) : !snapshot ? (
        <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
          {t("Could not load this child's information.")}
        </div>
      ) : (
        <>
          {/* Student header */}
          <div className="bg-white rounded-lg shadow p-4">
            ...
            {snapshot.enrollment?.classMaster && (
              <p className="text-xs text-gray-500">{t('Class master')}: {snapshot.enrollment.classMaster}</p>
            )}
            <div className="grid grid-cols-2 gap-2 mt-3">
              <div className="bg-blue-50 rounded-lg px-3 py-2 text-center">
                <p className="text-lg font-bold text-blue-700">{avg(snapshot.academic?.overallAverage)}</p>
                <p className="text-[11px] text-blue-700">{t('Overall average / 20')}</p>
              </div>
              <div className="bg-gray-50 rounded-lg px-3 py-2 text-center">
                <p className="text-lg font-bold text-gray-700">{snapshot.academic?.totalAssessments ?? 0}</p>
                <p className="text-[11px] text-gray-600">{t('Assessments recorded')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-gray-900">{t('Report Cards')}</h2>
            </div>
            {reports.filter(r => r.status === 'COMPLETED').length === 0 ? (
              <p className="p-4 text-sm text-gray-400">{t('No report cards available yet.')}</p>
            ) : (
                  ...
                  <li key={r.id} className="px-4 py-2.5 flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900">
                        {t('Sequence')} {r.sequenceNumber}{r.termName ? ` — ${r.termName}` : ''}
                      </p>
                      <p className="text-[11px] text-gray-400">
                        {r.academicYearName || ''}{r.generatedAt ? ` · ${t('generated')} ${new Date(r.generatedAt).toLocaleDateString()}` : ''}
                      </p>
                    </div>
                    <button
                    ...
                    >
                      ...
                      {downloadingSeq === r.examSequenceId ? t('Downloading…') : t('Download PDF')}
                    </button>
                  </li>
                ))}
              </ul>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-gray-900">{t('Results by Sequence')}</h2>
            </div>
            {(snapshot.academic?.sequences || []).length === 0 ? (
              <p className="p-4 text-sm text-gray-400">{t('No marks recorded yet.')}</p>
            ) : (
              <ul className="divide-y divide-gray-50">
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-sm font-medium text-gray-900">
                            {t('Sequence')} {seq.sequenceNumber}{seq.termName ? ` — ${seq.termName}` : ''}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-xs shrink-0">
                          <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded-full font-semibold">{avg(seq.average)}</span>
                          {summary?.rank != null && (
                            <span className="text-gray-500">{t('Rank')} {summary.rank}/{summary.totalStudents ?? '—'}</span>
                          )}
                          {summary?.decision && (
                            ...
                            {seq.subjects.map(sub => (
                              <li key={sub.subjectId} className="py-1.5 flex items-center justify-between gap-2 text-sm">
                                <div className="min-w-0">
                                  ...
                                  <span className="text-[11px] text-gray-400"> {sub.coefficient ? `· ${t('coef')} ${sub.coefficient}` : ''}{sub.teacher ? ` · ${sub.teacher}` : ''}</span>
                                </div>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-gray-900">{t('Discipline')} ({snapshot.discipline?.totalIssues ?? 0})</h2>
            </div>
            {(snapshot.discipline?.issues || []).length === 0 ? (
              <p className="p-4 text-sm text-gray-400">{t('No discipline issues — keep it up! 🎉')}</p>
            ) : (
              <ul className="divide-y divide-gray-50">
                {(snapshot.discipline?.issues || []).map(i => (
                  <li key={i.id} className="px-4 py-2.5">
                    ...
                    {i.actionTaken && <p className="text-xs text-green-700">{t('Action')}: {i.actionTaken}</p>}
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
              ...
              <h2 className="font-semibold text-sm text-gray-900">{t('Health')}</h2>
            </div>
            <div className="p-4 space-y-2">
              ...
              {(snapshot.health?.recentVisits || []).length === 0 ? (
                <p className="text-sm text-gray-400">{t('No infirmary visits recorded.')}</p>
              ) : (
                <ul className="divide-y divide-gray-100">
                  {(snapshot.health?.recentVisits || []).map(v => (
                    <li key={v.id} className="py-2">
                      <p className="text-sm text-gray-800">
                        {v.reason}
                        {v.sentHome && <span className="ml-2 text-[11px] px-2 py-0.5 bg-red-50 text-red-700 rounded-full">{t('Sent home')}</span>}
                      </p>
                      <p className="text-[11px] text-gray-400">
