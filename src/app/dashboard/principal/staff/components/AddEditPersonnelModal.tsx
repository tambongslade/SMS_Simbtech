        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
            <div className="relative mx-auto p-8 border w-full max-w-2xl shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="space-y-4">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-gray-700">{t('Full Name')} *</label>
                    ...
                    {!initialData && ( // Only show password for new users
                        <div>
                            ...
                            <input type="password" name="password" id="password" value={formData.password || ''} onChange={handleChange} required className="mt-1 block w-full input-field" placeholder={t('Min. 6 characters')} disabled={isLoading} />
                        </div>
                    )}
                    <div>
                    ...
                    <div>
                        <label htmlFor="date_of_birth" className="block text-sm font-medium text-gray-700">{t('Date of Birth')} *</label>
              ...
              For this example, direct role editing is not part of this form. */}
                    ...
                    <div className="flex justify-end space-x-3 pt-5 border-t mt-5">
                        ...
                        <button type="submit" className="px-4 py-2 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700" disabled={isLoading}>
                            {isLoading ? t('Saving...') : (initialData ? t('Update Personnel') : t('Create Personnel'))}
                        </button>
                    </div>
                </form>
            </div>
