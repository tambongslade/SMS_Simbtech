...
export default function PrincipalTeacherManagementPage() {
  const { t } = useLanguage();
  ...
  const [timetablePdfTeacherId, setTimetablePdfTeacherId] = useState<number | null>(null);

  const handleDownloadTimetablePdf = async (teacher: TeacherSearchItem) => {
    setTimetablePdfTeacherId(teacher.id);
    try {
      await downloadTeacherTimetablePdf(teacher.id, teacher.name, selectedAcademicYear?.id);
    } finally {
      setTimetablePdfTeacherId(null);
    }
  };
    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <TeacherTable
          ...
          getTeacherHref={(teacher) => `/dashboard/principal/teacher-management/${teacher.id}`}
