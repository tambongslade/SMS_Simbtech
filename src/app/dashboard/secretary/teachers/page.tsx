...
export default function SecretaryTeachersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!form.name || !form.email || !form.gender || !form.date_of_birth || !form.password) {
      toast.error(t('Name, email, gender, date of birth and password are required.'));
      return;
    }
    ...
    try {
      ...
      if (message !== 'Unauthorized') {
        toast.error(message || t('Failed to create teacher.'));
      }
    } finally {
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{t('Teachers')}</h1>
          <p className="text-sm text-gray-600 mt-1">{t('Create and view teacher accounts.')}</p>
        </div>
        <Button
          color="primary"
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title={t('Add Teacher')} size="lg">
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label={t('Full Name *')} name="name" value={form.name} onChange={handleFormChange} required />
            <Input label={t('Email *')} name="email" type="email" value={form.email} onChange={handleFormChange} required />
            <Input label={t('Phone')} name="phone" value={form.phone} onChange={handleFormChange} />
            <Select
              label={t('Gender *')}
              name="gender"
              value={form.gender}
              ...
              options={[
                { value: '', label: t('Select gender') },
                { value: 'MALE', label: t('Male') },
                { value: 'FEMALE', label: t('Female') },
              ]}
            />
            <Input label={t('Date of Birth *')} name="date_of_birth" type="date" value={form.date_of_birth} onChange={handleFormChange} required />
            <Input label={t('Address')} name="address" value={form.address} onChange={handleFormChange} />
            <Input label={t('Temporary Password *')} name="password" type="password" value={form.password} onChange={handleFormChange} required />
          </div>
          <p className="text-xs text-gray-500">
            {t('The teacher will be created and assigned the Teacher role for')}
            {selectedAcademicYear ? ` ${selectedAcademicYear.name}` : ` ${t('the current academic year')}`}.
          </p>
