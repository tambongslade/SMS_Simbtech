    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900">{t('Academic Years')}</h1>
          <button
        ...
        {fetchError && fetchError.message !== 'Unauthorized' && !isLoadingData && (
          <div className="mb-6 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
            ...
            <span className="block sm:inline"> {t('Failed to load academic years. Please try refreshing.')}</span>
          </div>
        )}
        ...
        <div className="space-y-6">
          {isLoadingData && academicYears.length === 0 && <p className="text-center text-gray-500">{t('Loading academic years...')}</p>}
          {!isLoadingData && !fetchError && academicYears.length === 0 && !isCreating && (
            <div className="text-center py-10 bg-white rounded-lg shadow-md">
              ...
              <p className="mt-1 text-sm text-gray-500">{t('Get started by creating a new academic year.')}</p>
            </div>
          )}
              ...
              <div key={year.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${year.isActive ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                        {year.isActive ? t('Active') : t('Inactive')}
                      </span>
                    </div>
                    <div className="flex space-x-2 rtl:space-x-reverse">
                      ...
                      <button
                        ...
                        title={year.isActive ? t('Cannot delete active year') : t('Delete Academic Year')}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <button
                        ...
                        title={year.isActive ? t('Set as Inactive') : t('Set as Active')}
                      >
                        {year.isActive ? (
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {year.terms && year.terms.length > 0 && (
                  <div className="border-t border-gray-200 px-4 py-3">
                    <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('Terms:')}</h3>
                          ...
                          <span>
                            ...
                            {term.isHoliday && (
                              <span className="ml-2 px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full bg-amber-100 text-amber-800">
                                {t('Holiday')}{term.classIds && term.classIds.length > 0 ? ` · ${term.classIds.length} ${term.classIds.length === 1 ? t('class') : t('classes')}` : ''}
                              </span>
                            )}
                          </span>
                          {!term.isHoliday && term.feeDeadline && (
                            <span>({t('Fee Deadline')}: {formatDateDisplay(term.feeDeadline)})</span>
                          )}
                        </li>
                      ))}
                    </ul>
