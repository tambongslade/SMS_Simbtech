    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                {/* Updated Title */}
                <h1 className="text-2xl font-bold text-gray-900 mb-6">{t('Discipline Master Management')}</h1>
                ...
                <div className="bg-white rounded-lg shadow-sm">
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            ...
                            {!isLoading && disciplineMasters.length === 0 && (
                                <tr><td colSpan={4} className="text-center py-4 text-gray-500">{t('No Discipline Masters found.')}</td></tr>
                            )}
                                ...
                                <tr key={dm.id}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-gray-700">
                                                {dm.assignedSubClassIds?.length || 0} {t('subclasses')}
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        ...
                                        <button
                                            ...
                                            disabled={isLoading}
                                            title={t('Edit Discipline Master')}
                                        >
                    ...
                    <div className="md:hidden divide-y divide-gray-100">
                        {isLoading && (
                            <div className="text-center py-4 text-gray-500 italic">{t('Loading...')}</div>
                        )}
                        {!isLoading && disciplineMasters.length === 0 && (
                            <div className="text-center py-4 text-gray-500">{t('No Discipline Masters found.')}</div>
                        )}
                        {disciplineMasters.map((dm) => (
            ...
            {isAssignmentModalOpen && selectedDm && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
                    <div className="relative mx-auto p-8 border w-full max-w-2xl shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
                        {/* Updated Title */}
                        <h3 className="text-lg font-medium leading-6 text-gray-900 mb-4">{t('Assign Subclasses to')}: {selectedDm.name}</h3>
