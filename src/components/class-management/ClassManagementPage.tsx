...
import { SubclassDetailsModal } from './components/SubclassDetailsModal';
import { PlusIcon, PencilIcon, TrashIcon, BuildingLibraryIcon, UsersIcon, XMarkIcon, BookOpenIcon } from '@heroicons/react/24/outline';
  ...
  const Modal: React.FC<{ isOpen: boolean; onClose: () => void; children: React.ReactNode }> = ({ isOpen, onClose, children }) => {
    ...
    return (
      <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
        <div className="relative mx-auto p-6 border w-full max-w-xl shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
                      ...
                      {cls.subClasses.map((sub) => (
                        <li key={sub.id} className="p-3 flex justify-between items-center hover:bg-gray-50">
                          <div
                            ...
                            onClick={() => openSubclassDetailsModal(sub)}
                          ...
                          <div className="flex space-x-1">
                            <button onClick={() => sub.id && openSubjectManagementModal({ type: 'subclass', id: sub.id, name: sub.name })} className="p-1 text-green-500 hover:text-green-700 disabled:opacity-50" title="Manage Subjects" disabled={isLoadingMutation || !sub.id}>
      ...
      <ManageSubjectsModal
        ...
        onClose={closeSubjectManagementModal}
        onSave={handleAssignSubjects}
        allSubjects={allSubjects}
        assignedSubjects={assignedSubjects}
        targetName={subjectManagementTarget?.name || ''}
        isLoading={isLoadingMutation}
      />

      {/* Subclass Details Modal */}
      <SubclassDetailsModal
        isOpen={isSubclassDetailsModalOpen}
        onClose={closeSubclassDetailsModal}
        subclass={selectedSubclassForDetails}
        isLoadingSubjects={isLoadingSubclassDetails}
        onManageSubjects={(subclass) => {
          closeSubclassDetailsModal();
          if (subclass.id) {
            openSubjectManagementModal({ type: 'subclass', id: subclass.id, name: subclass.name });
          }
        }}
        onAssignClassMaster={(subclass) => {
          closeSubclassDetailsModal();
          // Find the parent class name for the subclass
          const parentClass = classes.find(cls => cls.subClasses?.some(sub => sub.id === subclass.id));
          if (parentClass) {
            openEditSubclassModal(subclass, parentClass.name);
          }
        }}
      />
    </div>
